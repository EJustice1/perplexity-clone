...
class TrafilaturaContentExtractor(ContentExtractorProviderInterface):
    """Trafilatura library implementation of the content extractor provider interface."""
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        logger.info(
            "Initialized Trafilatura content extractor provider"
        )
    ...
    async def extract_content(
        ...
        try:
            ...
            logger.debug(
                f"Trafilatura extraction successful for {request.url}"
            )
            ...
            logger.error(
                f"Trafilatura extraction error for {request.url}: {str(e)}",
                exc_info=True,
            )
            return ContentExtractionResult(
                url=request.url,
    ...
    async def extract_content_batch(
        ...
        try:
            ...
            logger.info(
                f"Trafilatura batch extraction completed: {successful_extractions}/{len(requests)} successful in {extraction_time:.2f}s"
            )
            ...
            logger.error(
                f"Trafilatura batch extraction error: {str(e)}",
                exc_info=True,
            )
            return ContentExtractionResponse(
                results=[],
    ...
    def _extract_title_from_html(
        ...
        try:
            ...
            if h1_tag:
                ...
                return h1_text.strip() if h1_text else None
