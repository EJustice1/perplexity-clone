...
class Gemini2FlashLiteProvider(QueryEnhancementInterface):
    ...
    def __init__(self, api_key: str):
        ...
        self.model = (
            "gemini-2.0-flash-lite"  # Smallest, fastest model
        )
        self.timeout = 5.0  # Very short timeout

        if not api_key or not api_key.strip():
            logger.warning(
                "No API key provided for ultra-lightweight LLM provider"
            )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if "candidates" in data and data["candidates"]:
                    enhanced_text = data["candidates"][0]["content"][
                        "parts"
                    ][0]["text"]
                ...
                logger.warning(
                    "LLM enhancement failed, using original query"
                )
                return QueryEnhancementResponse(
            ...
            logger.warning(
                "LLM enhancement timed out, using original query"
            )
            return QueryEnhancementResponse(
                enhanced_query=request.original_query,
                success=False,
