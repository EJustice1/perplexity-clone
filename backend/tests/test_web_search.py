...
from backend.src.services.web_search import (
    WebSearchResult,
    WebSearchProvider,
...
class TestWebSearchServiceFactory:
    ...
    @patch('backend.src.services.web_search.settings')
    ...
    def test_create_web_search_service_missing_api_key(self, mock_settings):
        ...
        with pytest.raises(ValueError, match="SERPER_API_KEY environment variable is required for web search"):
            create_web_search_service()
    
    @patch('backend.src.services.web_search.create_web_search_service')
