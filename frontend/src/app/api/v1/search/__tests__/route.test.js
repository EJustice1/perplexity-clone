  ...
  describe('POST', () => {
    ...
    it('should use Docker service name when NODE_ENV is production', async () => {
      ...
      const mockBackendResponse = {
        ok: true,
        status: 200,
        json: jest.fn().mockResolvedValue({ result: 'You searched for: docker query' }),
        headers: new Map([['content-type', 'application/json']]),
      }
      global.fetch.mockResolvedValue(mockBackendResponse)
    ...
    it('should handle backend error responses', async () => {
      ...
      expect(data).toEqual({ error: 'Backend error: Bad Request: Query cannot be empty' })
    })
    ...
    it('should forward authorization headers when present', async () => {
      const mockBackendResponse = {
        ok: true,
        status: 200,
        json: jest.fn().mockResolvedValue({ result: 'You searched for: auth query' }),
        headers: new Map([['content-type', 'application/json']]),
      }
      global.fetch.mockResolvedValue(mockBackendResponse)
    ...
    it('should handle malformed JSON gracefully', async () => {
      ...
      expect(response.status).toBe(500)
      expect(data.error).toContain('Failed to communicate with backend service')
    })
  })
